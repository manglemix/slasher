...
func _input(event):
	if (event is InputEventMouseButton and event.button_index == BUTTON_LEFT) or (event is InputEventScreenTouch and event.index == _index):
		...
		var look_vector := Vector3(tmp_vector.x, tmp_vector.y, 0)
...
func _unhandled_input(event):
	if ((event is InputEventMouseButton and event.button_index == BUTTON_LEFT) or (event is InputEventScreenTouch)) and event.pressed:
		...
		show()
		player_movement.rotation_style = CharacterMovement.RotationStyles.FACE_ORIGIN
...
func _process(delta):
	...
	if _timer <= 0:
		set_process(false)
		player_movement.rotation_style = CharacterMovement.RotationStyles.FACE_MOVEMENT
